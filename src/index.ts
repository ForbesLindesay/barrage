...
export interface ReadableStream<T> {
  ...
  syphon<TStream extends WritableStream<T>>(destination: TStream, options?: { end?: boolean; }): TStream;
  buffer(): Promise<T[]>;
...
export class Readable<T = Buffer> extends streams.Readable implements ReadableStream<T> {
  private _push = (chunk: T | null, encoding?: string): boolean => this.push(chunk, encoding);
  ...
  pipe<TStream extends NodeJS.WritableStream>(destination: TStream, options?: { end?: boolean; }): TStream;
  pipe<TStream extends WritableStream<T>>(destination: TStream, options?: { end?: boolean; }): TStream;
  pipe<TStream extends WritableStream<T> | NodeJS.WritableStream>(destination: TStream, options?: { end?: boolean; }): TStream {
    return super.pipe((destination as any), options);
  }
  syphon<TStream extends NodeJS.WritableStream>(destination: TStream, options?: { end?: boolean; }): TStream;
  syphon<TStream extends WritableStream<T>>(destination: TStream, options?: { end?: boolean; }): TStream;
  syphon<TStream extends WritableStream<T> | NodeJS.WritableStream>(destination: TStream, options?: { end?: boolean; }): TStream {
...
export class Duplex<TWrite, TRead> extends streams.Duplex implements DuplexStream<TWrite, TRead> {
  private _push = (chunk: TRead | null, encoding?: string): boolean => this.push(chunk, encoding);
  ...
  pipe<TStream extends NodeJS.WritableStream>(destination: TStream, options?: { end?: boolean; }): TStream;
  pipe<TStream extends WritableStream<TRead>>(destination: TStream, options?: { end?: boolean; }): TStream;
  pipe<TStream extends WritableStream<TRead> | NodeJS.WritableStream>(destination: TStream, options?: { end?: boolean; }): TStream {
    return super.pipe((destination as any), options);
  }
  syphon<TStream extends NodeJS.WritableStream>(destination: TStream, options?: { end?: boolean; }): TStream;
  syphon<TStream extends WritableStream<TRead>>(destination: TStream, options?: { end?: boolean; }): TStream;
  syphon<TStream extends WritableStream<TRead> | NodeJS.WritableStream>(destination: TStream, options?: { end?: boolean; }): TStream {
    this.on('error', destination.emit.bind(destination, 'error'))
    return this.pipe((destination as any), options);
  }
...
export class Transform<TWrite, TRead> extends streams.Transform implements DuplexStream<TWrite, TRead> {
  private _push = (chunk: TRead, encoding?: string): boolean => this.push(chunk, encoding);
...
export class FlatMap<TWrite, TRead> extends Transform<TWrite, TRead> {
  ...
  constructor(transform: (chunk: TWrite) => TRead[] | PromiseLike<TRead[]> | ReadableStream<TRead>, options: FlatMapOptions = {}) {
    ...
    const opts = {
      ...
      transform: (chunk: TWrite, encoding: string, push: (chunk: TRead, encoding?: string) => boolean, callback: (err?: null | Error, chunk?: TRead) => void) => {
        available--;
        let called = false;
        if (available > 0) {
          callback();
        ...
        this._ready = this._ready.then<TRead[] | ReadableStream<TRead>>(() => {
          return result;
        }).then(result => {
          if (isReadableStream(result)) {
            return result.syphon(new Writable<TRead>({
      ...
      flush: (push: (chunk: TRead, encoding?: string) => boolean, callback: (err?: null | Error, chunk?: TRead) => void) => {
        this._ready.then(() => {
          const err = this._nextError;
          this._nextError = null;
          callback(err);
...
export class BufferTransform<TInput = Buffer | string, TOutput = Buffer | string> extends Transform<TInput, TOutput> {
  ...
  constructor(transform: (input: any) => TOutput | PromiseLike<TOutput>, encoding?: 'buffer' | 'utf8') {
    super({
